...
// CUDA kernel for matrix multiplication with coalesced memory access
__global__ void matrixMulCoalesced(float *A, float *B, float *C, int n) {
    ...
    for (int t = 0; t < (n + TILE_SIZE - 1) / TILE_SIZE; t++) {
        ...
        int a_col = t * TILE_SIZE + threadIdx.x;  // Coalesced access: consecutive threads access consecutive columns
        int b_row = t * TILE_SIZE + threadIdx.y;  // Coalesced access: consecutive threads access consecutive rows
